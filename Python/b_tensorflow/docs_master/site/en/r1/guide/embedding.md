...
```
blue:  (red, 47.6Åã), (yellow, 51.9Åã), (purple, 52.4Åã)
blues:  (jazz, 53.3Åã), (folk, 59.1Åã), (bluegrass, 60.6Åã)
orange:  (yellow, 53.5Åã), (colored, 58.0Åã), (bright, 59.9Åã)
oranges:  (apples, 45.3Åã), (lemons, 48.3Åã), (mangoes, 50.4Åã)
```

This would tell an application that apples and oranges are in some way more
similar (45.3Åã apart) than lemons and oranges (48.3Åã apart).
...
For example, the sentence ÅgI have a cat.Åh could be split into
`[ÅgIÅh, ÅghaveÅh, ÅgaÅh, ÅgcatÅh, Åg.Åh]` and then the corresponding `word_ids` tensor
...
```
word_embeddings = tf.get_variable(Ågword_embeddingsÅh,
...
*Selection of the nearest neighbors of ÅgimportantÅh in a word embedding dataset.*

Advanced tip: filtering with custom projection can be powerful. Below, we
filtered the 100 nearest neighbors of ÅgpoliticsÅh and projected them onto the
ÅgworstÅh - ÅgbestÅh vector as an x axis. The y axis is random. As a result, one
finds on the right side ÅgideasÅh, ÅgscienceÅh, ÅgperspectiveÅh, ÅgjournalismÅh but on
the left ÅgcrisisÅh, ÅgviolenceÅh and ÅgconflictÅh.
...
variable.  If the TSV metadata file has only a single column, then we donÅft
...
thumbnailsone for each vector in the embedding. A sprite image stores
