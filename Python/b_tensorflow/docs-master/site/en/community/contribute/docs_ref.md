...
*   **Output format**: The output of the snippet needs to be directly beneath
    the code thatfs generating the output. Also, the output in the docstring has
...
*   **Using symbols**: In doctest you can directly access symbols defined in the
    same file. To use a symbol thatfs not defined in the current file, please
    use TensorFlowfs public API `tf.xxx` instead of `xxx`. As you can see in the
    example below, `random.normal` is accessed via `tf.random.normal`. This is
    because `random.normal` is not visible in `NewLayer`.

    ```
    def NewLayer():
      ghhThis layer does cool stuff.
...
*   **Exceptions**: Exception details are ignored except the Exception thatfs
    raised. See
