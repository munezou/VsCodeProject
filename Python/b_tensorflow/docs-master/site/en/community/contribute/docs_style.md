...
*   *Use a casual and friendly voice.* Write TensorFlow documentation like a
    conversationas if you're talking to another person one-on-one. Use a
...
Use `# ÅÀ` instead of a single equal sign when you want to show what an op
returns.

```python
# 'input' is a tensor of shape [2, 3, 5] 
(tf.expand_dims(input, 0))  # ÅÀ [1, 2, 3, 5]
```
