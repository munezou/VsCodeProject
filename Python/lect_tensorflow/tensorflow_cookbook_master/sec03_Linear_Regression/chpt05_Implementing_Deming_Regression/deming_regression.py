...
def loss(input_x, aa, bb, targets):
    ...
    deming_numerator = tf.abs(tf.subtract(targets, tf.add(tf.matmul(input_x, aa), bb)))
    deming_denominator = tf.sqrt(tf.add(tf.square(aa),1))
...
# Declare optimizer
optimize = tf.keras.optimizers.SGD(0.15)
...
print   (
        '       finished         deming_regression.py                             ({0})             \n'.format(date_today)
    )
