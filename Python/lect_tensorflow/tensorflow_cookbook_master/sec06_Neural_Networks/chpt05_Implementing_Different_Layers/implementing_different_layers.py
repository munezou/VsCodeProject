...
# Generate 1D data
data_1d = tf.cast(np.random.normal(size=data_size), dtype=tf.float32)
...
my_filter = tf.cast(tf.Variable(tf.random.normal(shape=[1, conv_size, 1, 1])), dtype=tf.float32)

# Create convolution layer
my_convolution_output = conv_layer_1d(data_1d, my_filter, stride_size)
...
my_activation_output = activation(data_1d)
...
my_maxpool_output = max_pool(data_1d, maxpool_size, stride_size)
...
# Convolution Output
print('Input = array of length {}'.format(data_1d.shape.as_list()[0]))
print(
...
print('my_convolution_output(data_1d) = \n{0}\n'.format(my_convolution_output))

# Activation Output
print('\nInput = above array of length {}'.format(my_convolution_output.shape.as_list()[0]))

print(
        'ReLU element wise returns '
        'an array of length {}:'.format(
                                        my_activation_output.shape.as_list()[0]
                                    )
    )

print('my_activation_output = \n{0}\n'.format(my_activation_output))
...
print('my_maxpool_output = \n{0}\n'.format(my_maxpool_output))
...
print('my_full_output = \n{0}\n'.format(my_full_output))
...
data_2d = tf.cast(np.random.normal(size=data_size), dtype=tf.float32)
...
my_filter = tf.cast(tf.Variable(tf.random.normal(shape=[conv_size, conv_size, 1, 1])), dtype=tf.float32)

# Create Convolutional Layer
my_convolution_output = conv_layer_2d(data_2d, my_filter, conv_stride=conv_stride_size)
...
def max_pool(input_2d, width, height, stride):
    ...
    pool_output = tf.nn.max_pool2d(
                        input=input_4d, ksize=[1, height, width, 1],
...
my_maxpool_output = max_pool(
                        my_activation_output,
                        width=maxpool_size,
                        height=maxpool_size,
...
# Convolution Output
print('Input = {} array'.format(data_2d.shape.as_list()))
...
print('my_activation_output(data_2d) = \n{0}\n'.format(my_activation_output))
...
print(
    '      finished      implementing_different_layers.py          ({0})             \n'.format(date_today)
)
